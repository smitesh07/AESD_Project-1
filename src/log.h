 ...
 * 2) Reference macros LOG_XXX to log at specific thresholds.  Use printf style
 * argument
 ...
 *        code path
 *  3) Call logInit() once in the main init routine, before any logging is
 * attempted and after any timing capability for
 *  loggerGetTimestamp() is complete.
 *  For the Blue Gecko platform, logging can be viewed with Tera Term or similar
 * terminal emulator.
 *       Select the "JLink CDC UART Port" and baud rate 115200.
 *  All logging is off by default (compiled out of the build)
 *   * To turn on you should #define INCLUDE_LOGGING 1 in your project files or
 * setup a build configuration for
 *       this purpose.
 *  Even after turning logging on with INCLUDE_LOGGING, debug logging is off by
 * default.
 *   * To turn debug logging on for a specific .c file, #define
 * INCLUDE_LOG_DEBUG 1 at the top of the file
 *       before the #include "log.h" reference.
 *   * To turn on for all files #define INCLUDE_LOG_DEBUG 1 in the project
 * configuration.
 */
#ifndef LOG_ERROR
#define LOG_ERROR(message, ...) LOG_DO(message, "Error", ##__VA_ARGS__)
#endif

#ifndef LOG_WARN
#define LOG_WARN(message, ...) LOG_DO(message, "Warn ", ##__VA_ARGS__)
#endif

#ifndef LOG_INFO
#define LOG_INFO(message, ...) LOG_DO(message, "Info ", ##__VA_ARGS__)
#endif

#if INCLUDE_LOG_DEBUG
#ifndef LOG_DEBUG
#define LOG_DEBUG(message, ...) LOG_DO(message, "Debug", ##__VA_ARGS__)
